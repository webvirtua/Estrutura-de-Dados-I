...
int insere_Fila(Fila* fi, struct aluno al){
    ...
    if(Fila_cheia(fi))
        return 0;
    fi->final = (fi->final+1)%MAX; //%MAX faz com que quando chegar no ultimo elemento da fila ele insere no inicio do vetor
...
int tamanho_Fila(Fila* fi){
    ...
    //printf("Final: %d, Inicio: %d, Quantidade: %d | ", fi->final , fi->inicio, qtd);
    return qtd;
}
